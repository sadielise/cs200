...
public class BinaryTree<T extends Comparable> extends BinaryTreeBasis<T> {
	...
	private int getLeaves(TreeNode<T> rootNode){
		...
		if(rootNode.getLeft() == null && rootNode.getRight() == null){
			...
			return getLeaves(rootNode.getRight());
		}
		
		else if(rootNode.getRight() == null){
			return getLeaves(rootNode.getLeft());
		}

		// recursive case
		else{
			return getLeaves(rootNode.getLeft()) + getLeaves(rootNode.getRight());
		}
	}

	// returns true if binary tree satisfies the heap property
	public Boolean isHeap(){

		return checkHeap(root);
	}

	// helper method for isHeap
	public Boolean checkHeap(TreeNode<T> rootNode){
		...
		// test right
			...
			if(rootNode.getRight().getItem().compareTo(rootNode.getItem()) > 0){
				return false;
			}
			
			else{
				return checkHeap(rootNode.getRight());
			}
		}
		...
		// test left
			...
			if(rootNode.getLeft().getItem().compareTo(rootNode.getItem()) > 0){
				return false;
			}
			
			else{
				return checkHeap(rootNode.getLeft());
			}
		}
		...
		// test both
			...
			if((rootNode.getLeft().getItem().compareTo(rootNode.getItem()) > 0) || (rootNode.getRight().getItem().compareTo(rootNode.getItem()) > 0)){
				return false;
			}
			
			else{
				return checkHeap(rootNode.getLeft()) && checkHeap(rootNode.getRight());
			}
